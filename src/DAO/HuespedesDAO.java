	...
	public void guardar(Huespedes huespedes) {
		try {
			...
			try (PreparedStatement pstm = con.prepareStatement(sql, java.sql.Statement.RETURN_GENERATED_KEYS)){
				pstm.setObject(1, huespedes.getNombre());
				pstm.setObject(2, huespedes.getApellido());
				pstm.setObject(3, huespedes.getFechaNacimiento());
				pstm.setObject(4, huespedes.getNacionalidad());
				pstm.setObject(5, huespedes.getTelefono());
	...
	private void transformarResultado(List<Huespedes> huespedes, PreparedStatement pstm) throws SQLException{
		try (ResultSet rst = pstm.executeQuery()){
			while(rst.next()) {
				...
				Huespedes listado = new Huespedes(id, nombre, apellido,fechaNacimiento,nacionalidad,telefono,idReserva);
				huespedes.add(listado);
			}
		}
	}
